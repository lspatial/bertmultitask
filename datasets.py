...
def synonym_replacement(sentence, n):
    ...
    for i in index:
        ...
        if synonyms:
            new_sentence[i] = synonyms[0]  # replace with the first synonym
            count += 1
        if count >= n:
            break
    return ' '.join(new_sentence)

def load_multitask_data(sentiment_filename,paraphrase_filename,similarity_filename,split='train',dataug=True,test=False):
    sentiment_data = []
    num_labels = {}
    if split == 'test':
        with open(sentiment_filename, 'r', encoding="utf-8-sig") as fp:
        ...
        with open(sentiment_filename, 'r', encoding="utf-8-sig") as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                ...
                if split=='train' and dataug and random.random()< 0.01:
                    sentiment_data.append((synonym_replacement(sent,3), label, sent_id+'_aug'))
                if split=='train' and test and len(sentiment_data)>100: break
        ...
        with open(paraphrase_filename, 'r', encoding="utf-8-sig") as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                try:
                    ...
                    if split=='train' and dataug and random.random()<0.4:
                        paraphrase_data.append((preprocess_string(record['sentence2']),
                                                preprocess_string(record['sentence1']),
                                                int(float(record['is_duplicate'])), sent_id+'_inv'))
                    if split=='train' and test and len(paraphrase_data) > 100: break
                except:
                    pass
    ...
    similarity_data = []
    if split == 'test':
        with open(similarity_filename, 'r', encoding="utf-8-sig") as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                sent_id = record['id'].lower().strip()
        ...
        with open(similarity_filename, 'r', encoding="utf-8-sig") as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):
                ...
                if split=='train' and dataug:
                    similarity_data.append((preprocess_string(record['sentence2']),
                                            preprocess_string(record['sentence1']),
                                            float(record['similarity']), sent_id+'_inv'))
                if split=='train' and test and len(similarity_data)>100: break
