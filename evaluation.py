...
def model_eval_sst(dataloader, model, device,sentiloss = 'mul_bentropy'):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        if sentiloss == 'both':
            ...
            preds = np.argmax(logits+logits2, axis=1).flatten()
        else:
...
def model_eval_para(dataloader, model, device):
    ...
    para_y_pred=np.array(para_y_pred)
    para_y_true=np.array(para_y_true)
    f1 = f1_score(para_y_true, para_y_pred, average='macro')
    acc = accuracy_score(para_y_true, para_y_pred)
    return acc, f1, para_y_pred, para_y_true, para_sent_ids

def model_eval_sim(dataloader, model, device):
    model.eval()  # Switch to eval model, will turn off randomness like dropout.
    sts_y_true = []
    sts_y_pred = []
    sts_sent_ids = []
    ...
    para_y_pred=np.array(sts_y_pred)
    para_y_true=np.array(sts_y_true)
    pearson_mat = np.corrcoef(sts_y_pred, sts_y_true)
    sts_corr = pearson_mat[1][0]
    return sts_corr, -1, para_y_pred, para_y_true, sts_sent_ids

# Evaluate multitask model on dev sets.
def model_eval_multitask(sentiment_dataloader,
                         paraphrase_dataloader,
                         sts_dataloader,
                         model, device,sentiloss = 'mul_bentropy'):
    model.eval()  # Switch to eval model, will turn off randomness like dropout.
