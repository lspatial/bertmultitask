...
from bertadaptor import BertModelWithAdaptor
import gc
import os
...
class MultitaskBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask,task_id):
        ...
        ### TODO
        if isinstance(self.bert, BertModelWithAdaptor):
...
def train_multitask(args):
    ...
    sst_dev_data, num_labels,para_dev_data, sts_dev_data = load_multitask_data(args.sst_dev,args.para_dev,args.sts_dev, split ='validation')
    ...
    if not args.no_train_cpal:
        berg_config = BertConfig()
        BertModelWithAdaptor.from_BertModel(model.bert,berg_config)
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer._optim if isinstance(optimizer,PCGrad) else optimizer,
                                                num_warmup_steps=0,
...
def test_multitask(args):
    ...
    with torch.no_grad():
        ...
        if not args.no_train_cpal:
            ...
            BertModelWithAdaptor.from_BertModel(model.bert,bert_config)
...
def get_args():
    ...
    parser.add_argument("--gradient_p",   type=str,
                        help='surgery: gradient surgery; vaccine: vaccine',
    ...
    parser.add_argument("--model_type", type=str,
                        help='vanilla, pal, adaptor',
                        choices=('vanilla', 'pal','adaptor'), default="adaptor")
    parser.add_argument("--sts_dev_out", type=str, default="predictions/sts-dev-output.csv")
    parser.add_argument("--sts_test_out", type=str, default="predictions/sts-test-output.csv")
    parser.add_argument("--no_train_cpal", action='store_true')
    parser.add_argument("--hidden_dropout_prob", type=float, default=0.3)
    parser.add_argument("--lr", type=float, help="learning rate", default=1e-5)
    parser.add_argument("--mul_bentropy", action='store_true',help="Multiple binary entropy loss for sentiment")
    parser.add_argument("--test_mode", action='store_true')
    parser.add_argument("--data_aug", action='store_true')
    parser.add_argument("--root", type=str, help="root path for training",
                        default='/adt_geocomplex/bertresult_multitasks_adaptorup')
...
if __name__ == "__main__":
    ...
    args.tarpath = retrievePath(args,other='_adaptor_pretrain')
