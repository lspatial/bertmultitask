...
from bertpalcom import BertModelWithPAL
import gc 
...
def batchloss_sentiment(batch, sharedspace, args,gradient_accs=1):
    ...
    if args.sentiloss == 'mul_bentropy':
        ...
        loss2 = F.cross_entropy(logits, b_labels.view(-1), reduction='mean')
        loss = loss1 + loss2 
    else:
        loss = F.binary_cross_entropy_with_logits(logits, multi_labels)
...
def loadPretrainedModel(args,device):
    ...
    if not args.no_train_cpal:
        ...
        BertModelWithPAL.from_BertModel(model.bert, bert_config,device)
...
def train_multitask(args):
    ...
    if args.pretrained_file is not None:
        ...
        if not args.no_train_cpal:
            bert_config = BertConfig()
            BertModelWithPAL.from_BertModel(model.bert, bert_config, device, train_pal=not args.no_train_cpal)
    model.to(device)
    lr = args.lr
        ...
        if args.gradient_p is not None:
            for i in tqdm(range(num_batches_per_epoch // 3), desc=f'Train {epoch}', disable=TQDM_DISABLE,
                ...
                for j, name in enumerate(['sst', 'sts', 'para']):
                    ...
                    torch.cuda.empty_cache()
                    gc.collect()
                if args.gradient_p is not None and args.gradient_p== 'vaccine':
                    optimizer.backward(losses)
                elif args.gradient_p is not None and args.gradient_p == 'surgery':
...
def get_args():
    ...
    parser.add_argument("--root", type=str, help="root path for training",
                        default='/adt_geocomplex/bertresult_multitasks_twostage')
