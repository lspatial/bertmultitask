...
from base_bert2 import BertPreTrainedModel
from utils import *
import math
...
class BertSelfAttention(nn.Module):
  def __init__(self, config, init_to_identity=False):
    ...
    self.attention_head_size_sq=self.attention_head_size**0.5
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    att_scores = att_scores + attention_mask  # att_scores = att_scores.masked_fill(attention_mask == 0, -1e9)
    att_scores = att_scores/self.attention_head_size_sq
    att_scores = F.softmax(att_scores, dim=-1)
    att_scores = self.dropout(att_scores)
    att_scores = torch.matmul(att_scores, value)
    att_scores = att_scores.transpose(1,2).contiguous()
    att_scores = att_scores.view(att_scores.shape[0], att_scores.shape[1], self.all_head_size)
    return att_scores
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    tmpout=dropout(output)
    tmpout=dense_layer(tmpout)
    tmpout=ln_layer(tmpout+input)
    return tmpout
    #raise NotImplementedError
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    ### TODO
    mhead_atts=self.self_attention(hidden_states,attention_mask)
    adnorm_out=self.add_norm(hidden_states, mhead_atts,  self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    dense_out=self.interm_af(self.interm_dense(adnorm_out))
    foutput=self.add_norm(adnorm_out, dense_out,  self.out_dense, self.out_dropout, self.out_layer_norm)
    return foutput
    #raise NotImplementedError
...
#    raise NotImplementedError
 ...
 #   raise NotImplementedError
    ...
    ### TODO
    all_embeds = input_embeds + pos_embeds + tk_type_embeds
    all_embeds=self.embed_layer_norm(all_embeds)
    all_embeds=self.embed_dropout(all_embeds)
    return all_embeds
   # raise NotImplementedError
